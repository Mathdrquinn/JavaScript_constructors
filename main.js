...
var virtualPerson1;
var virtualPerson2;
var personArr = [virtualPerson, virtualPerson1, virtualPerson2]

console.log("In this game there are three types of people: Person, Theif, Hacker.");
console.log("If you choose to be a Person you must select the following: name, pocket cash, stored cash, and your risk factor (scale1-10).");
...
function Account(inputPerson) {
  ...
  this.recalculateAccount = function (person) {
    // this.checking = Math.round((inputPerson.storedCash * (inputPerson.riskFactor/20)) * 100) / 100,
    // this.savings = Math.round((inputPerson.storedCash * (inputPerson.riskFactor/20)) * 100) / 100,
    // this.secure = Math.round((inputPerson.storedCash * ((10 - inputPerson.riskFactor) / 10)) * 100) / 100,
    this.total = Math.round((this.checking + this.savings + this.secure) * 100) / 100;
    person.storedCash = this.total;
  };
}

//Constructor to create people
function Person(inputName, inputPocketCash, inputStoredCash, inputRiskFactor) {
  this.name = inputName,
  ...
  this.invest = function() {
    var invest;
    //Tell player how much they have and ask for a bet.
    invest = prompt("You have $" + this.accountInfo.savings + " in your savings.  How much would you like to invest?");
    ...
    var returns = 0;
    if (guess === luckyNumber) {
      returns = invest * luckyNumber;
      console.log("Its a bull market out there. You just won $" + returns + "!");
    }
    //If they are within 1 of the right number, they win 1:1
    else if (guess + 1 === luckyNumber || guess - 1 === luckyNumber) {
      returns = invest;
      console.log("Decent returns.  You won $" + returns + "!");
    }
    //If they lose subtract bet from pocketCash
    else {
      returns = bet * -1;
    ...
    this.accountInfo.savings += returns;
...
function Theif(inputAlias, inputSkill, inputPocketCash) {
  ...
  this.steal = function (targetPerson) {
    ...
    if (this.skill > luck) {
      if (purse >= targetPerson.pocketCash) {
        purse = targetPerson.pocketCash;
...
/////////////////////////////////////////////
/////////////////////////////////////////////
//Functions and methods to mess with the page

var typeProperties = function (type) {
    if (type === "Person") {
      $("#riskForm").css("display","none");
      $("#skillForm").css("display","none");
      $("#riskForm").slideToggle();
      console.log("risk");
    }
    else if (type === "Theif" || type === "Hacker") {
      $("#riskForm").css("display","none");
      $("#skillForm").css("display","none");
...
//actions to load when page is ready
$(document).ready(function() {
  ...
  $('.rating').on("change", 'input', function() {
    score = $(this).val();
    console.log(score);
    $(this).closest('.rating').find('span').text(score);
  });
  $('form').on('submit', function (event) {
    event.preventDefault();
    console.log('here');
    var name = $(this).closest('form').find('#name');
    var pocket = $(this).closest('form').find('#name');
    var stored = $(this).closest('form').find('#name');
    var type = $(this).closest('form').find('.constructType').val();
    var rating = $(this).closest('form').find('.riskFactor').val();
    virtualPerson = new Person(name, pocket, stored, rating);
    $('form').after(virtualPerson.pocketCash);
    });
});
